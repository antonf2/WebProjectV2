...
import { DeleteUser } from "../MISC/api";
    ...
    <>
      <nav className="navbar navbar-expand align-items-center d-flex bd-highlight ">
        ...
        <div className="relative ml-3">
          <div>
            <button type="button" className="relative flex rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800" onClick={toggleProfileLinks}>
          ...
          <div className={`absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none ${profileMenuOpen ? 'block' : 'hidden'}`} >
            <a href="/profile" className="block px-4 py-2 text-sm text-gray-700">Your Profile</a>
            <a onClick={logOut} href="/login" className="block px-4 py-2 text-sm text-gray-700">Sign out</a>
          </div>
        </div>
